...
module.exports = (function() {
    ...
    api.verify = function(text, config, filename, saveState) {
        ...
        return verifyInternal(ast, text, config, filename, saveState);
    };

    /**
     * verify an AST that has already been generated.
     * The provided ast must include tokens, comments etc. to match the output produced
     * by esprima in the standard parsing path.
     * @param {string} ast The previously generated AST/parse result.
     * @param {string} text The JavaScript text to verify.
     * @param {Object} config An object whose keys specify the rules to use.
     * @param {string=} filename The optional filename of the file being checked.
     *      If this is not set, the filename will default to '<input>' in the rule context.
     ...
     * @returns {Object[]} The results as an array of messages or null if no messages.
     */
    api.verifyPreparedAST = function(ast, text, config, filename, saveState) {
        // todo: we can add a bit more validation here
        if (text === null || text === undefined) {
            messages.push({
                fatal: true,
                severity: 2,
                message: "Must provide text as well as AST",
                line: 0,
                column: 0
            });
            return messages;
    ...
    function verifyInternal(ast, text, config, filename, saveState) {
        var ecmaFeatures,
            ecmaVersion;
