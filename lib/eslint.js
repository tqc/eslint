...
module.exports = (function() {
    ...
    /**
     * Parses text into an AST. This is equivalent to calling parser.parse with the following exceptions:
     *  - Errors are returned rather than thrown
     *  - Empty input returns null
     *  - Shebangs are ignored
     * @param {string} text The text to parse.
     * @param {Object} config The ESLint configuration object.
     * @returns {ASTNode|Error} The AST if successful or the error object if not.
     * @private
     */
    api.parse = function(text, config) {

        var parser, cleanedText, shebang, parseResult;

        // if the code contains a shebang, comment it because it is valid without being parsable as JS
        cleanedText = text.replace(/^#!([^\r\n]+)/, function(match, captured) {
            shebang = captured;
            return "//" + captured;
        });

        // no input returns no output
        ...
        try {
            ...
            parseResult = parser.parse(cleanedText, {
                loc: true,
                range: true,
                raw: true,
                tokens: true,
            ...
            // remove shebang comments that don't really exist in the source code
            if (shebang && parseResult && parseResult.comments && parseResult.comments.length && parseResult.comments[0].value === shebang) {
                parseResult.comments.splice(0, 1);

                if (parseResult.body.length && parseResult.body[0].leadingComments && parseResult.body[0].leadingComments[0].value === shebang) {
                    parseResult.body[0].leadingComments.splice(0, 1);
                }
            }
            return parseResult;
     ...
     * @param {string|SourceCode} sourceCode Either the JavaScript text to verify or a SourceCode object containing
     *      the code and a pregenerated AST
     ...
     */
    api.verify = function(sourceCode, config, filename, saveState) {
        ...
        if (typeof sourceCode === "string") {
            sourceCode = new SourceCode(sourceCode, api.parse(sourceCode, config));
        } else if (!sourceCode instanceof SourceCode) {
            messages.push({
                fatal: true,
                severity: 2,
        ...
        currentText = sourceCode.text;
        currentSourceCode = sourceCode;

        var parseErrors = sourceCode.validate();
        if (parseErrors.length) {
            // parsing failed
            messages.push.apply(messages, parseErrors);
            return messages;
        }

        // if espree failed to parse the file, there's no sense in setting up rules
        if (!sourceCode.parseResult) {
            return messages;
        }

        currentAST = sourceCode.parseResult;

        // parse global comments and modify config
        config = modifyConfigsFromComments(filename, sourceCode.parseResult, config, reportingConfig, messages);
        ...
        scopeManager = escope.analyze(sourceCode.parseResult, {
            ignoreEval: true,
        ...
        currentTokens = createTokenStore(sourceCode.parseResult.tokens);
        Object.keys(currentTokens).forEach(function(method) {
            api[method] = currentTokens[method];
        });

        // augment global scope with declared global variables
        addDeclaredGlobals(sourceCode.parseResult, currentScopes[0], currentConfig);
         ...
         */
        controller.traverse(sourceCode.parseResult, {
    ...
    /**
     * Gets the current SourceCode object - text + AST
     * @returns {SourceCode} A SourceCode object containing the original
     * text and the result of parsing it.
     */
